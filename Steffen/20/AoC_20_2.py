...
with open('dotfile.txt', 'w') as dfile:
    dfile.write('digraph {\n')
    for m in modules.values():
        if isinstance(m, FlipFlop):
            dfile.write(f'{m.name} [label="FF-{m.name}"]\n')
        elif isinstance(m, Conjunction):
            dfile.write(f'{m.name} [label="CON-{m.name}" fillcolor="green" style="filled"]\n')
        else:
            dfile.write(f'{m.name} [shape="rect" fillcolor="yellow" style="filled"]\n')
    for m in modules.values():
        for o in m.outputs:
            dfile.write(f'{m.name} -> {o.name}\n')
    dfile.write('}\n')

# genauer Blick in die "Schaltung"
# wann sind fh, fn, hh, lk das erste Mal alle "LOW"
# das erste Mal für jedes Modul finden und multiplizieren!
...
while True:
    ...
    while len(pulses) > 0:
        ...
        count[p] += 1
        m.Receive(i,p)
        if m.name == 'fh' and p == LOW:
            print('fh: ', run)
        if m.name == 'fn' and p == LOW:
            print('fn: ', run)
        if m.name == 'hh' and p == LOW:
            print('hh: ', run)
        if m.name == 'lk' and p == LOW:
            print('lk: ', run)
            break
